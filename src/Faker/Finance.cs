...
namespace Faker
{
    public static class Finance
    {
        ...
        public static (string BankAccountNumber, string SortCode) UkBankDetails(bool formatSortCode = false)
        {
            var bankAccount = RandomNumber.Next(0, 99999999).ToString("00000000", CultureInfo.InvariantCulture);
            var sortCode = RandomNumber.Next(0, 999999).ToString("000000", CultureInfo.InvariantCulture);
            var formattedSortCode = formatSortCode ?
                string.Format("{0}-{1}-{2}", sortCode.Substring(0, 2), sortCode.Substring(2, 2), sortCode.Substring(4, 2)) :
                sortCode;

            return (bankAccount, formattedSortCode);
        }
